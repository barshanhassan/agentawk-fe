...
const AgencyWhiteLabelSettings = () => {
  const [agencyColor, setAgencyColor] = React.useState("#149f8f");
  const { mode } = useTheme();
        ...
        <CardContent className="p-0">
          <Tabs defaultValue="features" className="w-full">
            ...
            <TabsContent value="favicon" className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                <div className={cn("w-16 h-16 border-2 border-dashed rounded-lg flex items-center justify-center relative group cursor-pointer hover:border-primary transition-colors",
                  mode === "dark" ? "bg-[#1e293b] border-slate-700" : "bg-slate-50 border-slate-200")}>
                  ...
                  <PopoverTrigger asChild>
                    <div className={cn("border rounded-md p-2 flex items-center gap-3 cursor-pointer transition-colors",
                      mode === "dark" ? "bg-[#1e293b] border-slate-700 hover:bg-[#334155]/50" : "bg-slate-50 border-slate-200 hover:bg-slate-100")}>
                       <div className="w-5 h-5 rounded" style={{ backgroundColor: agencyColor }} />
                       <span className={cn("text-sm font-medium flex-1", mode === "dark" ? "text-gray-300" : "text-slate-700")}>{agencyColor}</span>
                       <ChevronDown size={14} className="text-gray-500" />
                    </div>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-3 bg-white border-none shadow-2xl rounded-lg overflow-hidden" align="start">
                    <div className="space-y-4">
                      <HexColorPicker color={agencyColor} onChange={setAgencyColor} />
                      
                      <div className="flex items-center gap-4">
                         <div className="w-8 h-8 flex items-center justify-center text-gray-600">
                            <Pipette size={18} />
                         </div>
                         <div className="w-10 h-10 rounded-full border border-gray-100" style={{ backgroundColor: agencyColor }} />
                        ...
                        {["R", "G", "B"].map((label, idx) => (
                          <div key={label} className="space-y-1 text-center">
                            <div className="bg-gray-50 border border-gray-200 rounded p-2 text-sm text-gray-700 font-medium">
                              {parseInt(agencyColor.slice(1 + idx * 2, 3 + idx * 2), 16) || 0}
                            </div>
            ...
            <TabsContent value="domain" className="p-6 space-y-8">
              <div className="flex items-center gap-2 max-w-2xl">
                 ...
                 <button className="bg-[#3b82f6] hover:bg-[#2563eb] text-white px-8 py-2.5 rounded-md text-sm font-bold transition-colors h-11">
                    Connect
                 </button>
              </div>
